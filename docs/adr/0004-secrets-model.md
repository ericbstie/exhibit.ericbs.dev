...
> **Deferred (ponytail):** `# ponytail: injectable profile hardcoded to production, add the .exhibit/config.toml override when a second app needs a different one`
...
> **Deferred (ponytail):** `# ponytail: single age key with no rotation tooling in v1, add ex key rotate/retire-old on the first real rotation or key-compromise event`
